...
class CreatePasswordPrompt(Controller):

    def __init__(self):
        Controller.__init__(self, "create-password.ui")
        self.ok_button.grab_default()

    def password_changed(self,widget):
        if self.password.get_text() == "":
            self.ok_button.set_sensitive(False)
            self.password_not_matching_label.set_visible(False)
        if self.password.get_text() != self.password1.get_text():
            self.ok_button.set_sensitive(False)
            self.password_not_matching_label.set_visible(True)
        else:
